...
def run_cycle():
    ...
    for file_info in files_to_analyze:
        ...
        if len(issues) > 0:
            try:
                from nexus_github import nexus_analysis_to_issues
                issue_urls = nexus_analysis_to_issues(repo, {
                    "session_id": session_id,
                    "issues": issues,
                    "verdict": verdict,
                }, path)
                if issue_urls:
                    sys.stderr.write(f"[AUTONOMOS] Created {len(issue_urls)} GitHub issues\n")
            except Exception as e:
                sys.stderr.write(f"[AUTONOMOS] Issue creation failed: {e}\n")
